...
def load_imdb_data(num_words, sequence_length, test_size=0.25, oov_token=None):
    # read reviews
    reviews = []
    with open("data/reviews.txt", errors="ignore") as f:
        for review in f:
            review = review.strip()
            reviews.append(review)

    labels = []
    with open("data/labels.txt", errors="ignore") as f:
        for label in f:
            label = label.strip()
            labels.append(label)
    ...
    data["int2label"] =  {0: "bugfix", 1: "information_giving", 2: "featurerequest"}
    data["label2int"] = {"bugfix": 0, "information_giving": 1, "featurerequest": 2}
